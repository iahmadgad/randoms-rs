...
use crate::{choice::Choice, mode::Mode, util::mreadln};
        ...
        while self.points1 < self.max_points && self.points2 < self.max_points {
            match &self.mode {
                ...
                Mode::Multiplayer(p1, p2) => {
                    self.last_choice1 =
                        Choice::parse(&mreadln(format!("\x1b[0;34m{p1}:\x1b[8m ").as_str()));
                    self.last_choice2 =
                        Choice::parse(&mreadln(format!("\x1b[0;31m{p2}:\x1b[8m ").as_str()));
                }
            }
            self.update_points();
